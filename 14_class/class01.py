# pass는 파이썬에서 아무 작업도 수행하지 않는 빈 코드 블록을 만들때
...
class Car:
    def __init__(self, model, price): #생성자, 객체가 생성될 때 자동으로 실행
        self.model = model #self는 현재 생성되는 객체 자신을 의미
        self.price = price
        print(f"모델 이름:{self.model}, 가격: {self.price} 객체 생성!")

    def __del__(self): #소멸자, 객체가 메모리에서 삭제될 때 사용
        print(f"{self.model}의 객체가 소멸됨!")

    def drive(self, speed, distance): #speed, distance는 외부에서 전달받는 매개변수
        # self는 이 메소드를 호출하는 객체가 자기 자신을 참조
        print(f"{self.model}가 {speed}km/h의 속도로 {distance}km만큼 전진")
