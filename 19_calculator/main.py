...
entry = tk.Entry(root, font=font.Font(size=30))
entry.grid(row=0, column=0, columnspan=10)

def btn_click(text): #현재 entry 문자열을 들고와서 변수에 담아둠
...
def equal_sign():
    try:
        num = eval(str(entry.get()))
    except SyntaxError:
        messagebox.showerror("오류", "알맞지 않은 수식")
        entry.delete(0, tk.END)
    else:
        entry.delete(0, tk.END)
...
for (text, row, col) in btn_list:
    (tk.Button(root, text=text, font=font.Font(size=28), command=lambda t=text:btn_click(t))
     .grid(row=row, column=col, ipadx=20))
